...
int main(){
    ...
    vector<int> numbers = {1,4215,6,126,624362,362,6232362,36,32326,235,2,8468,52424,242};

    cout << count_if(numbers.cbegin(), numbers.cend(), bind(greater<int>(), placeholders::_1, 1024)) << endl;

    vector<string> words = {"pooh", "pooh", "derp", "pooh", "honey", "balls"};

    cout << *(find_if(words.cbegin(), words.cend(), bind(not_equal_to<string>(), placeholders::_1, "pooh"))) << endl;

    vector<int> multiply2 = {1,2,3,4,5,6,7,8,9,0,9,8,7,6,5,4,3,2,1};

    transform(multiply2.begin(), multiply2.end(), multiply2.begin(), bind(multiplies<int>(), placeholders::_1, 2));

    for(const auto& c : multiply2)
        cout << c << " ";
