...
class getLine{
public:
    getLine(istream& is = cin): input(is) { }
    string operator()() const;
private:
    istream& input;
};

string getLine::operator()() const { string s; return getline(cin, s) ? s : ""; }

int main(){


    vector<string> standardInp;
    while(cin)
        standardInp.push_back(getLine()());

    for(const auto& c : standardInp)
        cout << c << " ";
