...
public:
    Bound() = default;
    Bound(unsigned l, unsigned u): lower(l), upper(u) { }
    bool operator()(const string& s){ return (s.size() >= lower) && (s.size() <= upper);}
private:
    unsigned lower = 0;
    unsigned upper = 0;
};
...
int main(){
    ...
    if(!input){
        cerr << "Could not find test.txt" << endl;
        return -1;
    }

    pair<unsigned, unsigned> subsets = {0,0};
    ...
    cout << "Size between 1 and 9: " << subsets.first << endl;
    cout << "Size of 10 or more: " << subsets.second << endl;
