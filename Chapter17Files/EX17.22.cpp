...
bool valid(const smatch& m){
    if(m[1].matched)
        return (m[3].matched) && (!m[5].matched);
        // !m[5].matched - since the | subexpression must be matched, this is equivalent
        // to saying that [ ]* was matched, and so we have zero or more whitespaces.
    else
        return (!m[3].matched) && (m[5].str() == m[8].str());
        // m[5].str() == m[8].str() - so either both are ., both are -, or both are the empty string
        // if both are the empty string that must mean both [ ]* got matched.

}

int main(){

    vector<PersonInfo> people = {{"dave", {"999999   9993", "(090)     999-9294"}},
                                 {"bill", {"(090)9232932", "928    3  94       2222"}},
                                 {"charlie", {"(842)023-2102", "039.029.3023"}}};

    string pattern = "(\\()?(\\d{3})(\\))?(([-.])|[ ]*)(\\d{3})(([-.])|[ ]*)(\\d{4})";

    //(([-.])|[ ]*) - either -, ., or any number of white s. This subexpression must be matched.
