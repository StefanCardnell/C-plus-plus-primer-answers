...
is the root of the issue. As a result of opening in binary, we cannot use formatted IO operations
and must resort to C-style strings (ugh..........................................) */
...
int main(){
    ...
    for(char line[100]; (inOut.tellg() != end_mark) && inOut.getline(line, 100, '\n');){ //assumes lines are only 99 characters long

        auto read_mark = inOut.tellg();
        inOut.seekp(0, fstream::end);

        cnt += strlen(line) + 1; //lines in binary mode end in \r\n, since the \n gets discarded the lines end in \r.
        const char* out = to_string(cnt).c_str();
        inOut.write(out, strlen(out));
