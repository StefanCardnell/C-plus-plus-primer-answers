...
int main(){
    ...
    cout << "Correct American Telephone Format." << endl;

    cout << "(Anything not classed as valid or invalid was not recognised as an American phone number)" << endl;

    string pattern = "(\\()?(\\d{3})(\\))?([-. ])?(\\d{3})([-. ])?(\\d{4})";

    regex r(pattern);

    cout << "\nPlease enter an American telephone number: ";

    for(string word; getline(cin, word); ){
        for(sregex_iterator it(word.begin(), word.end(), r), end; it != end; ++it){
            if(valid(*it)) cout << "Valid Phone Number: " << (it->str()) << endl;
            else cout << "Invalid Phone Number: " << (it->str()) << endl;
        }
        cout << "\nEnter another telephone number: " << endl;
    }
