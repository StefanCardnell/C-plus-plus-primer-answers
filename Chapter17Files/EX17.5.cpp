...
typedef pair<vector<vector<Sales_data>>::size_type, pair<vector<Sales_data>::const_iterator, vector<Sales_data>::const_iterator>> matches;
...
vector<matches> findStores(const vector<vector<Sales_data>>& stores, const string& book){
    ...
    for(auto it = stores.cbegin(); it != stores.cend(); ++it){
        ...
        if(iterPair.first != iterPair.second)
            ret.push_back(make_pair(it-stores.cbegin(), iterPair));
    }
    return ret;
}
...
ostream& processBooks(istream& is, ostream& os, const vector<vector<Sales_data>>& allSales){
    ...
    while(is >> book){
        ...
        for(const auto& store : Stores)
            os << "Store: " << store.first << " ... Sold: " << accumulate(store.second.first, store.second.second, Sales_data(book)) << endl;
    }
    return os;
}
