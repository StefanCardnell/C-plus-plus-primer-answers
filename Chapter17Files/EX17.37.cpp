...
int main(){
    ...
    ifstream input("EX17.37input.txt");

    char c[10]; /*Default-initialized. With is.getline c always gets a null character put at the end of the input.
                  So cout << c only outputs up to this null.*/

    while(input.getline(c, 10, '\n')){
        cout << c;
    }
    ...
    The output is "testwhoknows". These are the steps involved:

    1. Reads test up to the implicit newline, discards the newline.
    2. Reads a newline again and discards it, c set to null array.
    3. Same again
    4. Reads who knows.
    5. Reads 9 characters and appends null to last. failbit gets set and there is not output.
    */
