...
void biggies(vector<string>& words, string::size_type sz){
    ...
    cout << (it - words.begin()) << " words found with size greater than/equal to " << sz << " and they are:" << endl;

    for_each(words.begin(), it, [](const string& s1){ cout << s1 << " ";});

}

void biggiesStable(vector<string>& words, string::size_type sz){
    elimDups(words);
...
int main(){
    ...
    vector<string> v1(words), v2(words);
