...
int main(int argc, char* argv[]){
    ...
    std::ofstream output1(argv[2]), output2(argv[3]);
    ...
    std::istream_iterator<int> iter_istr(input), eof;
    std::ostream_iterator<int> iter_ostr1(output1, " "), iter_ostr2(output2, "\n");

    for_each(iter_istr, eof, [&](const int& i){ (i%2 == 0 ? iter_ostr2 : iter_ostr1) = i;});
}
