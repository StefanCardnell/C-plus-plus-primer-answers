...
int main(){
    ...
    int count = authors.count(desireAuth);
    ...
    map<string, set<string>> bookAuth;

    for(const auto& c : authors)
        bookAuth[c.first].insert(c.second);

    for(const auto& c : bookAuth){
        cout << c.first << ": ";
    ...
    if(!authors.empty()) author = authors.begin()->first;
    ...
    while(true){
        ...
        for(const auto& c : books)
            cout << "\"" << c << "\" ";
        cout << endl;
        if(pos.second != authors.end()) author = pos.second->first;
        else break;
    }*/ //original more shit way
