...
template <typename T> ostream& test(const T& c, ostream& os){
    for(typename T::size_type it = 0; it < c.size(); ++it)
        os << c[it] << " ";
    return os;
}

template <typename T> ostream& testAgain(const T& c, ostream& os){
...
int main(){
    ...
    list<int> a = {1,2,4,1,5,5,1,5,1,4,2};
