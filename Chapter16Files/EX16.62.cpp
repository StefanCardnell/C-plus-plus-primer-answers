...
int main(){
    ...
    unordered_multiset<Sales_data> a;

    a.insert(Sales_data("george", 2, 2));
    a.insert(Sales_data("george", 2, 2));
    a.insert(Sales_data("george", 2, 2));
    a.insert(Sales_data("kind", 2, 3));
    a.insert(Sales_data("kind", 2, 3));
    a.insert(Sales_data("kind", 2, 3));
    a.insert(Sales_data("kind", 2, 3));
    a.insert(Sales_data("stephen", 10, 15));
    a.insert(Sales_data("stephen", 10, 15));
    a.insert(Sales_data("stephen", 10, 15));
    a.insert(Sales_data("playful", 3, 4));
    a.insert(Sales_data("george", 3, 4));

    for(const auto& c : a)
        cout << c << " with hashcode " << hash<Sales_data>()(c) << endl;
